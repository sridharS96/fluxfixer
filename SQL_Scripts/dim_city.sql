...
SELECT
    ...
    , COALESCE(dim_state_province.continent_name, 'Invalid') AS continent_name
    , COALESCE(dim_state_province.region_name, 'Invalid') AS region_name
    , COALESCE(dim_state_province.subregion_name, 'Invalid') AS subregion_name
FROM
    dim_city__add_undefined AS dim_city
LEFT JOIN
