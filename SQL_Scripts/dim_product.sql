...
SELECT
  ...
  COALESCE(
    ...
    -1
  ) AS primary_contact_person_key,
  COALESCE(
    dim_supplier.primary_contact_full_name,
    'Invalid'
  ) AS primary_contact_full_name,
  COALESCE(
    dim_supplier.alternate_contact_person_key,
    -1
  ) AS alternate_contact_person_key,
  COALESCE(
    dim_supplier.alternate_contact_full_name,
    'Invalid'
  ...
  COALESCE(
    dim_supplier.state_province_key,
    -1
  ) AS delivery_state_province_key,
  COALESCE(
    dim_supplier.state_province_name,
    'Invalid'
  ) AS delivery_state_province_name,
  ...
  COALESCE(
    dim_package_type_outer.package_type_name,
    'Invalid'
  ) AS outer_package_type_name,
...
FROM
  dim_product__add_undefined AS dim_product
