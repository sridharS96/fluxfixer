...
WITH dim_customer_membership__source AS (
    SELECT
        *
    FROM
        `vit-lam-data.wide_world_importers.external__customer_membership`
)
...
, dim_customer_membership__rename_recast AS (
    ...
    FROM
        dim_customer_membership__source
)

, dim_customer_membership__handle_null AS (
    SELECT
        customer_id
        , COALESCE(membership, 'Undefined') AS membership
        , begin_effective_date
        , end_effective_date
    FROM
        dim_customer_membership__rename_recast
)
...
, dim_customer_membership__add_undefine AS (
    SELECT
        ...
        , end_effective_date
    FROM
        dim_customer_membership__handle_null
...
SELECT
    ...
    , end_effective_date
FROM
    dim_customer_membership__add_undefine
