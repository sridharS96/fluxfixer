...
, fact_customer_snapshot_by_month__generate_densed AS (
    ...
    WHERE dim_date.year_month BETWEEN dim_customer.start_month AND dim_customer.end_month
)
...
, fact_customer_snapshot_by_month__dense_snapshot AS (
    ...
    ON densed.customer_key = sales.customer_key AND densed.year_month = sales.year_month
)

, fact_customer_snapshot_by_month__calculate_measures AS (
    SELECT
        *,
        SUM(sales_amount) OVER (PARTITION BY customer_key
            ORDER BY year_month) AS lifetime_sales_amount,
        SUM(sales_order_quantity) OVER (PARTITION BY customer_key
            ORDER BY year_month) AS lifetime_sales_order_quantity,
        SUM(item_quantity) OVER (PARTITION BY customer_key
            ORDER BY year_month) AS lifetime_item_quantity,
        LAG(sales_amount, 1) OVER (PARTITION BY customer_key
            ORDER BY year_month) AS lm_sales_amount,
        SUM(sales_amount) OVER (PARTITION BY customer_key
            ORDER BY year_month
            ROWS BETWEEN 11 PRECEDING AND CURRENT ROW) AS l12m_sales_amount
...
, fact_customer_snapshot_by_month__calculate_percentile AS (
    SELECT
        *,
        PERCENT_RANK() OVER (PARTITION BY year_month
            ORDER BY sales_amount) AS monetary_percentile,
        PERCENT_RANK() OVER (PARTITION BY year_month
            ORDER BY lifetime_sales_amount) AS lifetime_monetary_percentile,
        PERCENT_RANK() OVER (PARTITION BY year_month
            ORDER BY lm_sales_amount) AS lm_monetary_percentile,
        PERCENT_RANK() OVER (PARTITION BY year_month
            ORDER BY l12m_sales_amount) AS l12m_monetary_percentile
